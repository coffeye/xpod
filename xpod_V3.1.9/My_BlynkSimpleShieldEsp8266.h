/**
 * @file       My_BlynkSimpleShieldEsp8266.h
...
#ifndef My_BlynkSimpleShieldEsp8266_h
#define My_BlynkSimpleShieldEsp8266_h
...
public:
    ...
    {      
        static unsigned long timeout = 3000; // Timeout in milliseconds (default: 5 seconds)
        static unsigned int maxAttempts = 10; // Maximum number of connection attempts (default: 10)
        config(esp8266, auth, domain, port); // Configure Blynk with provided parameters
        connectWiFi(ssid, pass); // Connect to Wi-Fi network

        unsigned int attempts = 0;
        unsigned long startTime = millis();

        // Keep trying to connect to Blynk server until successful or timeout
        while (!this->connect()) {
            if (attempts++ >= maxAttempts || millis() - startTime >= timeout) {
                // Maximum attempts reached or timeout occurred
